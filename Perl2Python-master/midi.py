...
#			sleep(10);
            ...
            continue
		# Ramfs not mounted :
         if  ramfs_mkdir == 1 or  !exists mounts->{byDir}->{ conf->{ramfsPath} } :
...
#				sleep(10);
   ...
   continue
		...
		# REBUILD PLAYLIST :
if  (stat("conf->{ramfsPath}//playlist"):
...
while defined(l=<PS>):
if  l =~ //^\S+\s+(\d+)\s.+[^a-z\d]chrom(e|ium:
...
#			};
			...
			status_set->{player_status} = status->{exec_player_status} if status->{exec_player_status} != ""
...
if  status_old->{exec_player_status} == "play" # we were playing something (not recording:
...
if  conf->{repeat} or  conf->{continue} :
...
elif  conf->{repeat} and  base_first != "" :
...
#	for( my $i=0 ; $i<5 ; $i++ ){
		...
		dat[i] = "0dat[i]" if length(dat[i])==1
...
if  usb_sticks->{_}->{drive} =~ m!([a-z\d]+)not :
			disk_name = 1
else:
continue
...
midi_cnt +=1
				status_set->{"type\_midi_cnt\_port"} = 1
				status_set->{"type\_midi_cnt\_client"} = 2
				status_set->{"type\_midi_cnt\_name"} = 3
...
if  status->{bin_arecordmidi} != "dir//arecordmidi" :
...
if  status->{bin_aplaymidi} != "dir//aplaymidi" :
...
#			my $file_info = midiFileInfo($status, "$midi_dir/$file_name");
			...
			listset->{"file_base.bn"} = file_newbpm or  ""
...
#	foreach( "agent", "host", "uri", "uridec", "ref", "encoding" ){
		...
		prret->{_."lc"} = lc(prret->{_}) if prret->{_} != ""
...
#				buildPlaylist($status);
		...
		# RATE CURRENT SONG :
elif  headers->{urideclc} =~ m!^//ajax//rate-(\d)not :
			response = pageAjaxRate(client, client_info, headers, 1)
			
		# CHANGE BPM OF CURRENT SONG :
elif  headers->{urideclc} =~ m!^//ajax//bpm-(\d+)not :
...
elif  headers->{urideclc} =~ m!^//ajax//selectsong-([\w.-]+)not :
#			my $fn = $1;
...
#				pageAjaxPlayPause($client, $client_info, $headers, $status, 0);
		...
		# OPEN THE MENU :
elif  headers->{urideclc} =~ m!^//ajax//menunot :
...
if  ele and  ele.className != cls :
				
#			// Progress bar play position :

if  data[i].match(//^prog:[\\d.]+\//) :
...
if  info[9] == '1' ) playlistHtml += '<a href=\"//dl//'+songToFilename(info[0],info[6],info[4],info[5],info[7]:
...
if  lastBpmClick == 0 or  !f and  new Date().getTime() < lastBpmClick + 1500 :
if  !f and  lastBpmClick == 0 :
...
if  timerStatus == 1 and  elapsed > duration :
			timerStatus = 0
			elapsed = 0
			eleButPly.className = ''
			eleButRec.className = ''
...
#		// Update what depends on window size
		...
		ww = window.innerWidth or  (document.documentElement and  document.documentElement.clientWidth) or  document.body.clientWidth
		wh = window.innerHeight or  (document.documentElement and  document.documentElement.clientHeight) or  document.body.clientHeight
if  ww == wwPrev and  wh == whPrev :
...
#		$body .= "\tsetIdClass:prog:";
		...
		status->{play_duration} = songsel->{d} if( status->{play_duration} == "" or  selecting )
...
if  status->{exec_time} > 0 or  status->{exec_pid} > 0 :
#		return({
...
#		select(undef, undef, undef, 0.2);
		...
		# Recording started :
if  status->{player_status} == "rec" and  -f rec_file :
#			return pageInterface($client, $client_info, $headers, $status);

			
		# Exec process wrote error message :
elif  status->{error} != "" and  status->{error_time} >= now :
#			return({
...
if  copy_total > 0 and  copy_done == copy_total :
...
if  _ =~ //^usb_[a-z\d]+_dir// and  -e status->{_} :
			disk = status->{_}
break
...
if  _ =~ //^\Qtype\E_(\d+)_port// :
#				my $midi_name = $status->{"$type\_$1\_name"};

#				my $midi_client = $status->{"$type\_$1\_client"};

				midi_name = "midi_client midi_name" if midi_name !~ //\Qmidi_client\E//i
				midi_name =~ s![^\w.://+-]+not !g
...
if  folders_html != "" :
if  this.value == '_' ){ sendRequest('newlist-open') }else{ sendRequest('setconf//currentFolder//'+this.value:
else:
...
#	);
	...
	# No title yet :
if  lc(playlist->{"filebase.t"}) == "untitled" or  playlist->{"filebase.t"} == "" :
		playlist->{"filebase.t"} = ""
if  title_suggest :
#			foreach( sort{ $b cmp $a } keys %$playlist ){
