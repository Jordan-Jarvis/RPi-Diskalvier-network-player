...
#define ALL_QUIET send_channel_mode(120, 0)
...
static int tempo_fst = 0; /* First tempo change event met */
static int tempo_new = 0; /* New tempo to replace first tempo change */
static int tempo_ppq = 384; /* PPQ (time division) so we can calculate duration based on tempo (wrong if tempo changes in middle of song) */
static float start_seconds = 0;
static int panic = 0;
static int noplay = 0;
			...
			case 0xff: /* meta event */
				...
				case 0x51: /* tempo */
					...
					if (smpte_timing) {
						...
						// NEXT TEMPO CHANGES ARE PROPORTIONAL TO RATIO FOUND AT FIRST TEMPO CHANGE :
						if( tempo_fst > 0 ){
							if( event->data.tempo > 0 ){
								event->data.tempo *= tempo_new / tempo_fst;
								printf("new_bpm = %.3f\n", (float) 60000000 / event->data.tempo);
							}
							
						// FIRST TEMPO CHANGE EVENT :
						}else{
							tempo_fst = event->data.tempo > 0 ? event->data.tempo : 60000000;
							if( !tempo_new > 0 ) tempo_new = tempo_fst;
							printf("original_bpm = %.3f\n", (float) 60000000 / tempo_fst);
							printf("new_bpm = %.3f\n", (float) 60000000 / tempo_new);
							event->data.tempo = tempo_new;
						}
						skip(len - 3);
					}
					break;
...
invalid_format:
	...
	int tempo_new_init = tempo_new > 0 ? tempo_new : 500000;
	...
	if (!smpte_timing) {
		...
		tempo_ppq = time_division;
		snd_seq_queue_tempo_set_tempo(queue_tempo, tempo_new_init);
		snd_seq_queue_tempo_set_ppq(queue_tempo, tempo_ppq);
	} else {
		...
		switch (i) {
		case 24:
			tempo_ppq = 12 * time_division;
			snd_seq_queue_tempo_set_tempo(queue_tempo, tempo_new_init);
			snd_seq_queue_tempo_set_ppq(queue_tempo, tempo_ppq);
			break;
		case 25:
			tempo_ppq = 10 * time_division;
			snd_seq_queue_tempo_set_tempo(queue_tempo, tempo_new_init/1.25);
			snd_seq_queue_tempo_set_ppq(queue_tempo, tempo_ppq);
			break;
		case 29: /* 30 drop-frame */
			tempo_ppq = 2997 * time_division;
			snd_seq_queue_tempo_set_tempo(queue_tempo, tempo_new_init);
			snd_seq_queue_tempo_set_ppq(queue_tempo, tempo_ppq);
			break;
		case 30:
			tempo_ppq = 15 * time_division;
			snd_seq_queue_tempo_set_tempo(queue_tempo, tempo_new_init);
			snd_seq_queue_tempo_set_ppq(queue_tempo, tempo_ppq);
			break;
		default:
...
static void play_midi(void)
{
	...
	float total_beats = (float) max_tick / tempo_ppq;
	printf("total_beats = %.3f\n", total_beats);
	
	float total_duration = total_beats * tempo_new / 1000000;
	...
	if( start_seconds > 0 ){
		/* REMINDER :
			tempo_new		= 60000000 / bpm
			tempo_ppq 		= ticks / beat
		*/
		tick_start = 1000000 * tempo_ppq * start_seconds / tempo_new;
	}
	...
	if( panic == 1 ){
		...
		/* FOR ALL EACH MIDI CHANNEL */
		for (int c = 0; c < 16; ++c) {
			
			/* SUSTAIN OFF */
			snd_seq_ev_set_fixed(&ev);
			ev.type =  SND_SEQ_EVENT_CONTROLLER;
			ev.data.note.channel = c;
	...
	for (;;) {
		...
		if( event->tick < tick_start ) continue;
		if( event->tick > max_tick ) break;
		
		/* output the event */
		ev.type = event->type;
		ev.time.tick = event->tick - tick_start;
	...
	ev.time.tick = max_tick - tick_start;
	ev.dest.client = SND_SEQ_CLIENT_SYSTEM;
	ev.dest.port = SND_SEQ_PORT_SYSTEM_TIMER;
	ev.data.queue.queue = queue;
	err = snd_seq_event_output(seq, &ev);
...
static void usage(const char *argv0)
{
	printf(
		...
		"OPTIONS ADDED BY FLORIAN BADOR:\n"
		"-b, --bpm=bpm               force initial bpm (all other tempo changes become proportional)\n"
		"-s, --start=seconds         start playing at specific position\n"
		"-c, --panic                 send panic before playing (NOT a full panic, only sustain+notes+sound off)\n"
		"-n, --noplay                do not play the file, only print info\n",
		argv0);
}
...
{
	static const char short_options[] = "hVlp:d:b:s:cn";
	...
	while ((c = getopt_long(argc, argv, short_options,
		...
		case 'd':
			...
			break;
		case 'b':
			if( atof(optarg) > 0 ) tempo_new = 60000000 / atof(optarg);
			break;
		case 's':
			start_seconds = atof(optarg);
			break;
		case 'c':
			panic = 1;
			break;
